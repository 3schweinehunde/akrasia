defmodule AkrasiaWeb.UserAuth do
  import Plug.Conn
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      AkrasiaWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def require_authenticated_admin_user(conn, _opts) do
    if conn.assigns[:current_user] && conn.assigns[:current_user].admin do
      ...
      conn
      |> put_flash(:error, "You must log in as an admin user to access this page.")
      |> maybe_store_return_to()
      |> redirect(to: Routes.user_session_path(conn, :new))
      |> halt()
    end
  ...
  defp signed_in_path(_conn), do: "/diagram"
end
